...
// 22800732
// 1475443 气浪
let time = 100 * 1000, startTime = 30 * 60 * 1000, isStart = false
let isSendNotice = false

function searchLiveBroadcast(id = '22800732' , groupId='1143106153') {
    const hour = new Date().getHours()
    if(hour<9){
        isStart = false
        isSendNotice = false
        setTimeout(() =>searchLiveBroadcast(id, groupId), 60*60*1000)
        return
    }
    if(hour < 17){
        time = 3*60*1000
    ...
    needle('GET', url + id, {}, {}).then(res => {
        ...
        setTimeout(() =>searchLiveBroadcast(id, groupId), isStart ? startTime : time)
    }).catch(() => {
        setTimeout(() =>searchLiveBroadcast(id, groupId), isStart ? startTime : time)
    })
}
...
module.exports = function () {
    searchLiveBroadcast()
    setTimeout(() => {
        searchLiveBroadcast('1475443', '297336992') //咕群
    }, 50000)
}
